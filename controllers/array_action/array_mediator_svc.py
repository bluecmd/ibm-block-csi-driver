...
class SVCArrayMediator(ArrayMediatorAbstract, VolumeGroupInterface):
    ...
    def _chvolumegroup(self, id_or_name, **cli_kwargs):
        try:
            ...
            logger.debug("Error running chvolumegroup -object_id {} {}".format(id_or_name, self._format_cli_args(cli_kwargs)))
    ...
    def _get_replication_mode(self, volume_group_id):
        ...
        # Does this come from partition? problem?
    ...
    def verify_host_partition(self, host_name, new_partition_name):
        ...
        if hasattr(cli_host, "partition_name") and cli_host.partition_name:
            current_partition = cli_host.partition_name
        return current_partition == new_partition_name
    ...
    def _generate_volume_group_response(self, cli_volume_group):
        ...
        return VolumeGroup(name=cli_volume_group.name,
                           ...
                           volumes=volumes,
                           partition_name=self._get_partition_name_of_volume_group(cli_volume_group))

    @register_csi_plugin()
    def create_volume_group(self, name, partition_name):
        self._create_volume_group(name, partition_name)
    ...
    def verify_volume_group_partition(self, volume_group, partition_name):
        if volume_group.partition_name != partition_name:
            raise ValidationException(messages.VOLUME_GROUP_PARTITION_MISMATCH.format(
                volume_group.name, volume_group.partition_name, partition_name))

    @register_csi_plugin()
    def delete_volume_group(self, volume_group_name):
        self._lsvolumegroup(volume_group_name, not_exist_err=True)
        self._rmvolumegroup(volume_group_name)

    @register_csi_plugin()
    def add_volume_to_volume_group(self, volume_group_name, volume_id, partition_name):
        volume_name = self._get_volume_name_by_wwn(volume_id)
        cli_volume = self._get_cli_volume(volume_name)
        if not partition_name and cli_volume.volume_group_name and cli_volume.volume_group_name != volume_group_name:
            raise array_errors.VolumeAlreadyInVolumeGroup(volume_id, cli_volume.volume_group_name)
        self._change_volume_group(partition_name, cli_volume.name, volume_group_name)
